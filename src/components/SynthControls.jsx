...
const PianoKeyboard = ({ onChange, current, onSelectActive }) => {
  const [selected, setSelected] = useState([])

  const toggleNote = (note) => {
    const updated = selected.includes(note) ? selected.filter((n) => n !== note) : [...selected, note]
    setSelected(updated)
    onChange(updated)
  }
  ...
  for (let oct = 1; oct <= 8; oct++) {
    for (const note of NOTES) {
      allNotes.push({ name: `${note}${oct}`, isBlack: note.includes("#") })
    }
  }
    ...
    <div className="piano-keyboard-container">
      <div className="piano-keyboard">
        {allNotes.map((note, index) => {
          const isActive = selected.includes(note.name)

          // Position black keys above their preceding white key
          const style = {}
          if (note.isBlack) {
          ...
          return (
            <button
              ...
              className={`piano-key ${note.isBlack ? "black-key" : "white-key"} ${isActive ? "active" : ""}`}
              style={{
                ...style,
                transform: `rotate(${(Math.random() - 0.5) * 1.5}deg) translateY(${Math.random() * 2 - 1}px)`,
              }}
            >
...
const SynthControls = () => {
  ...
  const handleStop = () => {
    synthRef.current.forEach((v) => {
      ;[v.oscSine, v.oscSquare, v.oscTriangle, v.oscSaw].forEach((o) => {
        o.stop()
        o.dispose()
      })
  ...
  const handleChangeChord = () => {
    ...
    const master =
      old[0]?.master || new Tone.Gain(Math.pow(10, amplitude / 20)).connect(analyserRef.current).toDestination()
  ...
  return (
    <div className="Body">
      <div className="synth-controls-container">
        <h2 className="synth-header">SYNTH</h2>

        <div className="canvas-knob-container">
          <div className="canvas-button-container">
            <canvas ref={canvasRef} width={600} height={200} className="canvas" />
            <div className="button-container">
              <button className="button" onClick={handleStart} disabled={activeNotes.length === 0}>
                Start Synth
              </button>
              <button className="button" onClick={handleStop} disabled={synthRef.current.length === 0}>
                Stop Synth
              </button>
              <button className="button" onClick={handleChangeChord} disabled={synthRef.current.length === 0}>
                Change
              </button>
            </div>
          </div>
        ...
        <div className="note-selector-item-container">
          <PianoKeyboard onChange={setChordTarget} current={activeNotes} onSelectActive={setActiveNotes} />
        </div>
      </div>
    </div>
  )
